...
class SingleDroneRewEnv(BaseSingleAgentAviary):
    def __init__(
        self,
    ...
    def _observationSpace(self):
        ...
        #         )
        #         # "observation": gym.spaces.Box(
        #         #     np.array([-np.inf, -np.inf, -np.inf, -1, -1, -1, -1, -np.inf, -np.inf, -np.inf]),
        #         #     np.array([np.inf, np.inf, np.inf, 1, 1, 1, 1, np.inf, np.inf, np.inf]),
        #         #     dtype=np.float32
        #         # )
        #     }
        ...
        return gym.spaces.Box(
            np.array([-np.inf, -np.inf, 0, -np.inf, -np.inf, -np.inf, -1, -1, -1, -1, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf]),
            np.array([np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, 1, 1, 1, 1, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf]),
            dtype=np.float32
        )
        # return gym.spaces.Box(
        #     np.array([-np.inf, -np.inf, 0, -np.inf, -np.inf, -np.inf, -1, -1, -1, -1, -np.inf, -np.inf, -np.inf]),
        #     np.array([np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, 1, 1, 1, 1, np.inf, np.inf, np.inf]),
        #     dtype=np.float32
        # )
    ...
    def _computeObs(self):
        ...
        return np.hstack([normalized_desired_goal, normalized_obs]).reshape(16,).astype(np.float32)
        # return np.hstack([normalized_desired_goal, normalized_obs]).reshape(13,).astype(np.float32)
    ...
    def _clipAndNormalizeState(self, state):
        ...
        # obs = np.hstack([normalized_pos, state[3:7], normalized_vel_xy, normalized_vel_z]).reshape(10,).astype(np.float32)
        return obs
    ...
    def _computeReward(self):
        ...
        goal_rew = (prev_dist-curr_dist)/self.init_dist
        self.last_pos = np.copy(self.state[:3])

        #change in action
        action_size = self.action_space.shape[0]
        chg_Action = self.last_action[:,:action_size] - self.stored_lastAction
        chg_actionMagnitude = LA.norm(chg_Action)
        action_rew = -chg_actionMagnitude
        ...
        #spin
        omega = self.state[13:16]
        spin_rew = -LA.norm(omega)
        ...
        return goal_rew + action_rew + rew_orient + spin_rew + time_rew
    ...
    def reset(self):
        ...
        ###### Reset last_action
        self.stored_lastAction= np.zeros(self.action_space.shape[0])
